 ...
 * {@link VendrediGHFragment.OnFragmentInteractionListener} interface
        ...
        ghJourViewModel.getAllJour().observe(this, new Observer<List<JoinGHJourStatutRef>>() {
            ...
            public void onChanged(@Nullable List<JoinGHJourStatutRef> joinGHJourStatutRefs) {
                if (joinGHJourStatutRefs != null) {
                    jour.setText(date(joinGHJourStatutRefs.get(4).getJour()));
                    statutJour.setText(joinGHJourStatutRefs.get(4).getNom());
                    if (joinGHJourStatutRefs.get(4).getRapport_complete())
                        rapportComplete.setVisibility(View.VISIBLE);
                    ghJourContactViewModel.setAllJourContactMutable(joinGHJourStatutRefs.get(4).getJour());
                }
            }
        });
