 ...
 * {@link MercrediGHFragment.OnFragmentInteractionListener} interface
...
public class MercrediGHFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        return inflater.inflate(R.layout.fragment_mercredi_gh, container, false);
    }
        ...
        ghJourViewModel.getAllJour().observe(this, new Observer<List<JoinGHJourStatutRef>>() {
            ...
            public void onChanged(@Nullable List<JoinGHJourStatutRef> joinGHJourStatutRefs) {
                if (joinGHJourStatutRefs != null) {
                    jour.setText(date(joinGHJourStatutRefs.get(2).getJour()));
                    statutJour.setText(joinGHJourStatutRefs.get(2).getNom());
                    if (joinGHJourStatutRefs.get(2).getRapport_complete())
                        rapportComplete.setVisibility(View.VISIBLE);
                    ghJourContactViewModel.setAllJourContactMutable(joinGHJourStatutRefs.get(2).getJour());
                }
            }
        });
